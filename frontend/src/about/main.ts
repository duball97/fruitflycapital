import './style.css'
import { AmbientLight, Box3, Color, DirectionalLight, Group, PerspectiveCamera, Scene, SRGBColorSpace, Vector3, WebGLRenderer } from 'three'
...
gltfLoader.load('/models/flybody/about-fly.glb', ({ scene: root }) => {
  const bounds = new Box3().setFromObject(root)
  const center = bounds.getCenter(new Vector3())
  const size = bounds.getSize(new Vector3())
  root.position.sub(center)
  model.add(root)
  model.scale.setScalar(0.105 / Math.max(size.x, size.y, size.z, 0.001))
  modelStatus.hidden = true
}, undefined, () => {
  modelStatus.textContent = 'FLYBODY PREVIEW UNAVAILABLE'
