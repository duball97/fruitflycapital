...
function render(data: Portfolio | null, status = 'CONNECTING') {
  ...
  app.innerHTML = `<header class="site-header"><a class="portfolio-brand site-brand" href="/" aria-label="FruitFly Capital home"><img src="/fruitfly-logo.png" alt="" /> <span>FRUITFLY CAPITAL</span></a><nav class="site-nav" aria-label="Primary navigation"><a href="/">Simulation</a><a href="/about/">About</a><a class="is-active" href="/portfolio/">Portfolio</a><a href="/#buy">Buy</a><a href="https://x.com/fruitflycap" target="_blank" rel="noreferrer">Community</a></nav><a class="social-link header-social" href="https://x.com/fruitflycap" target="_blank" rel="noreferrer" aria-label="FruitFly Capital on X"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M18.9 2H22l-6.77 7.74L23.2 22h-6.24l-4.89-6.39L6.48 22H3.36l7.24-8.28L2.8 2h6.4l4.42 5.84L18.9 2Zm-1.1 17.7h1.73L8.28 4.2H6.42L17.8 19.7Z" /></svg><span>@fruitflycap</span></a></header><div class="portfolio-title"><h1>FRUITFLY CAPITAL</h1><p>AUTONOMOUS BIOLOGICAL FUND</p></div>
    ...
    <section class="panel wallet-summary"><div><small>ROBINHOOD WALLET</small><code>${walletAddress}</code></div><div><small>NATIVE BALANCE</small><strong>${native(wallet.nativeBalance)}</strong></div><div><small>AVAILABLE AFTER GAS RESERVE</small><strong>${native(wallet.availableToTrade)}</strong></div><div><small>CHAIN</small><strong>Robinhood · ${walletChain}</strong></div><div><small>RPC STATUS</small><strong class="wallet-status ${wallet.status === 'error' ? 'is-error' : ''}">${walletStatus}</strong></div></section>
    ...
    <section class="grid"><article class="panel"><h2>BUY / HOLD / SELL EVENTS</h2>${events.length ? events.slice().reverse().slice(0, 20).map(item => `<div class="trade"><b>${String(item.type || '—')}</b><span>${String(item.flyId || (Array.isArray(item.flyIds) ? item.flyIds.join(', ') : '—'))}</span><span>${String(item.status || '—')}</span><span>${String(item.tokenAddress || item.reason || '—')}</span><span>price ${String(item.executionPrice ?? '—')}</span><span>gas ${String(item.gas ?? item.estimatedGas ?? '—')}</span><span>slip ${String(item.slippage ?? '—')}%</span></div>`).join('') : '<p class="muted">No autonomous events yet.</p>'}</article><article class="panel"><h2>PENDING EXECUTION</h2>${pendingExecution.length ? pendingExecution.map(item => `<div class="trade"><b>${String(item.status || '—')}</b><span>${String(item.side || '—').toUpperCase()} ${String(item.tokenSymbol || 'TOKEN')}</span><span>${String(item.flyIds || '—')}</span><a href="${isRealTxHash(item.txHash) ? `https://robinhoodchain.blockscout.com/tx/${item.txHash}` : '#'}" target="_blank" rel="noopener noreferrer">VIEW TX ↗</a></div>`).join('') : pending.length ? pending.slice().reverse().map(item => `<div class="trade"><b>${String(item.status || '—').toUpperCase()}</b><span>${String(item.side || '—')}</span><span>${String(item.tokenAddress || '—')}</span><span>${String(item.reason || '—')}</span></div>`).join('') : '<p class="muted">No pending execution.</p>'}</article></section>
