...
function render(data: Portfolio | null, status = 'CONNECTING') {
  ...
  app.innerHTML = `<header><div><a class="portfolio-brand" href="./"><img src="/fruitfly-logo.png" alt="" /> FRUITFLY CAPITAL</a><h1>FRUITFLY CAPITAL</h1><p>AUTONOMOUS BIOLOGICAL FUND</p></div><div class="badges"><span>${String(fund.mode || 'dry-run').toUpperCase()}</span><span>${status}</span></div></header>
