...
def _serialized(method):
    """Serialize access to a shared sqlite connection used by async workers."""
...
class FundLedger:
    def __init__(self, path: str | Path = "data/fund/fund.db") -> None:
        ...
        # A legacy event contains malformed UTF-8 in a TEXT value. Decode
        # damaged historical text losslessly enough for the ledger to remain
        # readable; do not discard the row or rewrite the source database.
        self.connection.text_factory = lambda value: value.decode("utf-8", "replace")
